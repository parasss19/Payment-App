...
const WalletTransactions = ({ showAllInCard = false }) => {
  const navigate = useNavigate();
  const { walletTransactions } = useContext(MyContext);
  ...
  return (
    <div className="bg-white/90 backdrop-blur-md rounded-xl border border-gray-200 shadow-sm p-6">
      ...
      <div className="flex items-center justify-between gap-3 mb-4 font-[Geist] pr-2">
        ...
        {/* If user in /addFunds component then click on viewAll open all trxns in card only 
        if user in /dashboard then click on viewAll redirect to /transactions */}
        {showAllInCard ? (
          <button
